    ...
    "render": {
        ...
        "elements": {
            ...
            "detail-group": function ($elem) {
                ...
                if(pyramid_data.data.chat_messages.length > 0) {
                    $elem.find('.detail-group-messages-label').show();
                }
            },
            ...
            "detail-user-level": function ($elem) {
                ...
                group_data.group_ratings.forEach(function (rating) {
                    if (rating.sid.toLowerCase() == user_details.sid.toLowerCase()) {
                        ratings.push(rating);
                    }
                });

                if(ratings.length > 0) {
                    $elem.find('.detail-user-level-ratings-label').show();
                }
